...
tblGrupos = $("#tblGrupos").DataTable({
    ajax: {
        ...
        url: base_url + "Grupos/listarGrupo",
        dataSrc: ''
    },
    columns: [{
        'data': 'grupo'
...
function registrarGrupo(e) {
    ...
    if (grupo.value == "") {
        ...
        Swal.fire({
            position: 'top-end',
            icon: 'error',
            title: 'Coloque el nombre del grupo',
...
function btnEditarGrupo(id) {
    //Toma el id del h5 del modal del index Usuario y sustituye el título
    document.getElementById("title").innerHTML = "Actualizar grupo";
...
function btnReingresarGrupo(id) {
    Swal.fire({
        title: '¿Está seguro de reingresar?',
        text: "¡El grupo no se reingresará de forma permanente, solo cambiará el estado a activo!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
